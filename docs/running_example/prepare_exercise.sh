...
starter_dir="${script_dir}/exercise/starter"
...
if (( $# == 1 )); then
    ...
    target_dir="$(mktemp -d "${TMPDIR:-/tmp}/temperature-exercise.XXXXXX")"
fi

cp -- "${reference_dir}/analysis_config.json" \
    "${reference_dir}/measurements.csv" \
    "$target_dir/"
