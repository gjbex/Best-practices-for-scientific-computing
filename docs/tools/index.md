# Tools

This section contains a list of tools that you may find useful
when doing software development in a scientific context.  Some
tools are general, i.e., they are useful for any programming
language, while others are specific to a particular language.

Before comparing individual tools, see [choosing development
tools](choosing_tools.md) for selection criteria, a decision table, and a short
project activity.

  * [Choosing development tools](choosing_tools.md)
  * [Agentic software-development tools](agentic_ai.md)
  * [General Tools](general.md)
  * [C](C.md)
  * [C++](C-plus-plus.md)
  * [Fortran](Fortran.md)
  * [Rust](Rust.md)
  * [Julia](Julia.md)
  * [Python](Python.md)
  * [R](R.md)
